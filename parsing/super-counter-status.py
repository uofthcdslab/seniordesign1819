...
def getIn(sf, df, sfName):
    ...
    coords = df[['Latitude', 'Longitude', 'Status']]
...
# use top 25 calls
statuses = ['All'] + list(allCalls['Status'].value_counts()[:25].index
)
# get all regions
...
columns = ['Status'] + list(coords['Region'].unique()) + ['Total']
print(columns)
...
for status in statuses:
    ...
    if status == 'All':
        ...
        selection = coords[coords['Status'] == status]

    # compute numbers for nature
    counts = selection['Region'].value_counts()
    countTab.append([status] + ([0] * (len(columns)-1)))
...
# write table to file
with open('status-region-counts.csv', 'w') as f:
